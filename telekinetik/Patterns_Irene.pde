public class Fireflies extends TenerePattern {
  public String getAuthor() {
    return "Irene";
  }
  
     
   final DiscreteParameter flyCount = new DiscreteParameter("NUM", 60, 1, 100);
   final CompoundParameter speed = new CompoundParameter("SPEED", 2, 0, 7.5); 
   final CompoundParameter hue = new CompoundParameter("HUE", 0, 0, 360);
   
   private float radius = 40;
   private int numFireflies = 60;
   private List<Firefly> fireflies;
   private List<Firefly> queue;
   private SinLFO[] blinkers = new SinLFO[10];
   private LinearEnvelope decay = new LinearEnvelope(0,0,5000);
 ...
 // patternMode = PATTERN_MODE_FIRED;
     ...
     for (int i = 0; i < blinkers.length; ++i) {
       blinkers[i] = new SinLFO(0, 75, 1000  * (1.0f+(float)Math.random()*3.0f));      
       addModulator(blinkers[i]).setValue(Math.random()*50).start();
     }
  ...
  public void run(double deltaMs) {
    ...
    //  }
     ...
     for (int i = 0; i < fireflies.size(); ++i) { //remove fireflies while blinking off
      ...
      //if they fall off the end of the world, bring them back on the other side
     for (int i = 0; i < fireflies.size(); ++i) {
       if (fireflies.get(i).yPos > model.yMax + radius) {
           fireflies.get(i).yPos = model.yMin - radius;
       }
       if (fireflies.get(i).xPos > model.xMax + radius) {
           fireflies.get(i).xPos = model.xMin - radius;
       }
       if (fireflies.get(i).yPos < model.yMin - radius) {
           fireflies.get(i).yPos = model.yMax + radius;
       }
       if (fireflies.get(i).xPos < model.xMin - radius) {
           fireflies.get(i).xPos = model.xMax + radius;
       }
     }
     ...
     for (Firefly fly : fireflies) {
       ...
       for (LXPoint p :model.points) {
         ...
         if ( dist(fly.xPos,fly.yPos,  p.x , p.y) <= radius  && Math.abs(fly.theta - p.theta) <= radius) {
        
           float dist = dist(p.x, p.y, fly.xPos, fly.yPos);
           //float distSq = (float)Math.pow((LXUtils.wrapdistf(fly.theta, p.theta, 360)), 2) + (float)Math.pow(fly.yPos - p.y, 2) + (float)Math.pow(fly.xPos - p.x, 2);
           float brt = (float)Math.max(0, 100 - dist*4 - blinkers[fly.blinkIndex].getValuef());
           
           if (brt > LXColor.b(colors[p.index])) {
             
             colors[p.index] = LXColor.hsb(
               ((float)palette.getHue() + hue.getValuef()) % 360,
               100 - brt,
               brt
             );
           }
  ...
  private class Firefly {
     ...
     public float xPos = 0;
      public float yPos = 0;
     public LXVector velocity = new LXVector(0,0,0);
     public int blinkIndex = 0;

     public Firefly() {
       theta = (float)Math.random()*360;
       xPos = model.xMin + (float)Math.random()*model.xMax;
         yPos = model.yMin + (float)Math.random()*model.yMax;
       velocity = new LXVector((float)(Math.random()*2)-1 ,(float)(Math.random()*2)-1,0);
       blinkIndex = (int) Math.random()*blinkers.length;
       
      
     }

     public void move(float speed) {
       theta = (theta + speed * velocity.x + velocity.y) % 360;
 ...
 public class Voronoi extends LXPattern {
   ...
   final CompoundParameter width = new CompoundParameter("WIDTH", 0.3, 0.3, 1);
   final CompoundParameter hue = new CompoundParameter("HUE", 0, 0, 360);
   
   final int NUM_SITES = 25;
   ...
   private class Site {
     ...
     public Site() {
       theta = (float)Math.random() * 360;
       yPos = (float)Math.random() * (model.yMax);
       xPos = (float)Math.random() * (model.xMax);
       velocity = new LXVector((float)(Math.random()*2)-1 ,(float)(Math.random()*2)-1,0);
   ...
   public void run(double deltaMs) {
    ...
    // if (getChannel().getFader().getNormalized() == 0) return;
     ...
     for (LXPoint p : model.points) {
        ...
        colors[p.index] = 0;//reset each frame
       ...
       for (int i = 0; i < sites.length; ++i) {
         ...
         if (dist(sites[i].xPos, sites[i].yPos, p.x, p.y) < 150) { //restraint on calculation
           
           float wrap = LXUtils.wrapdistf(sites[i].theta, p.theta, 360);
  
           float distSq = ((float)Math.pow(wrap, 2)  +   (float)Math.pow(sites[i].yPos - p.y, 2)  + (float)Math.pow(sites[i].xPos - p.x, 2) )  ; // + (float)Math.pow(sites[i].xPos - p.x, 2);
          //   float distSq =  (float)dist( sites[i].yPos , sites[i].xPos , p.y, p.x ) ; // + (float)Math.pow(sites[i].xPos - p.x, 2);
       ...
       colors[p.index] = LXColor.hsb(
         ((float)palette.getHue() + hue.getValuef()) % 360,
         0,
         Math.max(0, Math.min(100, 100 - Math.sqrt(nextMinDistSq - minDistSq) / width.getValuef()))
       );
     }
...
//     for (LXPoint p : model.points) {
//       float wrapdistleft = LXUtils.wrapdistf(p.theta, spinf + (model.yMax - p.y) * coilf, 180);
//       float wrapdistright = LXUtils.wrapdistf(p.theta, -spinf - (model.yMax - p.y) * coilf, 180);
//       float width = yClimb.getValuef() + ((p.y - yHeight.getValuef())/model.yMax) * 50;
//       float df = Math.min(100, 3 * Math.max(0, wrapdistleft - width) + 3 * Math.max(0, wrapdistright - width));

//       colors[p.index] = lx.hsb(
//         (float)(palette.getHue() + .2f*p.y - 360) % 360, 
//         100.0, 
//         df
//       );
//     }
...
//     for (LXPoint p : model.points) {
//       float yn = (p.transformedY - model.yMin) / model.yMax;
//       float cBrt = 0;
//       float cHue = 0;
//       float flameWidth = flameSize.getValuef() / 2;
//       for (int i = 0; i < flames.size(); ++i) {
//         if (Utils.abs(flames.get(i).theta - p.transformedTheta) < (flameWidth * (1- yn))) {
//           cBrt = Utils.min(100, Utils.max(0, Utils.max(cBrt, (100 - 2 * Utils.abs(p.transformedY - flames.get(i).decay.getValuef()) - flames.get(i).decay.getBasisf() * 25) * Utils.min(1, 2 * (1 - flames.get(i).decay.getBasisf())) )));
...
//       for (LXPoint p : model.points) {
//         if (Utils.abs(bubble.theta - p.transformedTheta) < bubble.radius && Utils.abs(bubble.yPos - (p.transformedY - model.yMin)) < bubble.radius) {
//           float distTheta = LXUtils.wrapdistf(bubble.theta, p.transformedTheta, 360) * 0.8f;
//           float distY = bubble.yPos - (p.transformedY - model.yMin);
...
//               Utils.constrain(p.transformedY/model.yMax * 125 - 50 * (dist/bubble.radius), 0, 100)
//             );
//           }
//         }
//       }
...
// }
 ...
 //       for (int i = 0; i < sites.length; ++i) {
 //         if (Utils.abs(sites[i].yPos - p.transformedY) < 150) { //restraint on calculation
 //           float distSq = Utils.pow((LXUtils.wrapdistf(sites[i].theta, p.transformedTheta, 360)), 2) + Utils.pow(sites[i].yPos - p.transformedY, 2);
 //           if (distSq < nextMinDistSq) {
 //             if (distSq < minDistSq) {
 //               nextMinDistSq = minDistSq;
 //               minDistSq = distSq;
...
// class Pulley extends TSTriggerablePattern { //ported from Sugarps
...
//       for (LXPoint p : model.points) {
//         int gi = (int) Utils.constrain((p.x - model.xMin) * NUM_DIVISIONS / (model.xMax - model.xMin), 0, NUM_DIVISIONS-1);
//         float yn =  p.transformedY/model.yMax;
//         colors[p.index] = lx.hsb(
//           (lx.getBaseHuef() + Utils.abs(p.x - model.cx)*.8f + p.transformedY*.4f) % 360, 
//           Utils.constrain(100 *(0.8f -  yn * yn), 0, 100), 
//           Utils.max(0, 100 - Utils.abs(p.transformedY/2 - 50 - gravity[gi].getValuef())*falloff)
//         );
//       }
//     }
//   }
...
//       float yn =  p.transformedY/model.yMax;
//       float width = (1-yn) * 25;
//       float wrapdist = LXUtils.wrapdistf(p.transformedTheta, spinf + (p.transformedY) * 1/(gravity.getValuef() + 0.2f), 360);
...
// class Pulleys extends TSTriggerablePattern { //ported from Sugarps
//   private BasicParameter sz = new BasicParameter("SIZE", 0.5);
//   private BasicParameter beatAmount = new BasicParameter("BEAT", 0);
//   private BooleanParameter automated = new BooleanParameter("AUTO", true);
//   private BasicParameter speed = new BasicParameter("SPEED", 1, -3, 3);
...
//         float cHue = 0;
//         for (int j = 0; j < pulleys.size(); ++j) {
//           cHue = (lx.getBaseHuef() + Utils.abs(p.x - model.cx)*.8f + p.transformedY*.4f + pulleys.get(j).baseHue) % 360;
//           cBrt += Utils.max(0, pulleys.get(j).maxBrt.getValuef() * (100 - Utils.abs(p.transformedY/2 - 50 - pulleys.get(j).gravity.getValuef())*falloff));
//         }
//         float yn =  p.transformedY/model.yMax;
//         colors[p.index] = lx.hsb(
//           cHue, 
...
// //    for (LXPoint p : model.points) {
// //      p.transformedY = p.transformedY * ( 1 - ripple.getValuef() * amplitude.getValuef() * Utils.sin((p.transformedTheta + rotate) / 30 * Utils.PI ));
// //    }
// //  }
// //}
...
// //     for (LXPoint p : model.points) {
// //       float distVal = Utils.sqrt(Utils.pow((LXUtils.wrapdistf(thetaCenter, p.transformedTheta, 360)) * 0.8f, 2) + Utils.pow(yCenter - p.transformedY, 2));
// //       float heightHueVariance = 0.1f * p.transformedY;
// //       if (distVal < radius){
